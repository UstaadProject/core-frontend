...
const Signup = () => {
  ...
  const strength = password ? passwordReqs.filter((r) => r.test(password)).length : 0;
  const allMet = strength === 4;

  const strengthLabels = ['', 'Weak', 'Fair', 'Good', 'Strong'];
  // tone classes for the strength bar fill
  const strengthTone = [
    'bg-muted',
    'bg-destructive',
    'bg-streak',
        ...
        <CardContent className="p-7 sm:p-8">
          <div className="mb-7 space-y-1.5 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Start your web-dev journey today — it's free
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {password.length > 0 && (
              <div className="animate-fade-in space-y-2.5 rounded-xl border border-border bg-muted/40 p-3.5">
                <div className="flex items-center gap-2">
                  <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-muted">
                    <div
                      className={cn('h-full rounded-full transition-all duration-500', strengthTone)}
                      style={{ width: `${(strength / 4) * 100}%` }}
                    />
                  </div>
                  <span className={cn('w-10 text-right text-[11px] font-semibold', strengthTextTone)}>
